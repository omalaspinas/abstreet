...
impl HgtFile {
    pub fn new(
        ...
        if data.len() != expected_len {
            return Err(io::Error::new(
                io::ErrorKind::Other,
                format!(
                    "HgtFile: data size of {} is not equal to {}",
                    data.len(),
                    expected_len
                ),
            ));
    ...
    /// Creates a new HgtFile from a path, origin in latitude and longitude, and resolution in
    /// arc-seconds
    ...
    pub fn get(&self, row: u16, col: u16) -> Option<i16> {
        ...
        if row > 0 && row < samples && col > 0 && col < samples {
            let offset =
                (((samples - row - 1) as usize) * (samples as usize) + ((col - 1) as usize)) * 2;
